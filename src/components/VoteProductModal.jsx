...
export default function VoteProductModal({ setOpenVoteProduct }) {
    ...
    const handleSubmit = async () => {
        if (isConnected && address) {
            if (id && price) {
                const data = await voteProduct(id, price)
                console.log(data)

                if (data.status === "Success") {
                    toast.success(
                        `Voted for product with id ${id}! Txn hash is copied to clipboard.`,
                        {
                            position: "top-center",
                            autoClose: 2500,
            ...
            >
                ...
                <div className="flex flex-col items-center p-0 gap-[20px] w-full lg:w-4/5">
                    ...
                    <div className="mt-5 flex flex-row items-center justify-center gap-[20px] self-stretch">
                        <Button className="py-2 flex-1 text-base" onClick={handleSubmit}>Submit vote</Button>
                    </div>
                </div>
            </div>
        </div>
